    ...
    public function list(PaginateRequest $request)
    {
        try {
            ...
            $query = PaymentGateway::with('gatewayOptions')->where(function ($query) use ($requests) {
                foreach ($requests as $key => $request_val) {
                    if (in_array($key, $this->paymentGatewayFilter)) {
                        $query->where($key, 'like', '%' . $request_val . '%');
                    }

                    if (in_array($key, $this->exceptFilter)) {
                        $explodes = explode('|', $request_val);
            ...
            Log::info("PaymentGatewayService List Error: " . $exception->getMessage());
